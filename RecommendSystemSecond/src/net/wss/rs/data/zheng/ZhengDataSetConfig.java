package net.wss.rs.data.zheng;

public class ZhengDataSetConfig {
	//将数据库获得的数据文件，进行行处理，转换为三个需要的文件
	public static String AllRatingPath = "data/zhengdata/ratingall.txt";
	public static String AllDoctorPath = "data/zhengdata/doctorall.txt";
	public static String AllZhengPath = "data/zhengdata/zhengall.txt";
	//控制字符串分割的方式
	public static String AttrSplit = ";";
	//读取数据库获得的数据文件
	public static String AllDataPath = "data/zhengdata/alldata.txt";
	public static String SimiResultPath ="data/zhengdata/allsimi.txt";
}
